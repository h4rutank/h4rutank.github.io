---
title: 自作キーボードを作りましたとさ
date: 2026-03-15
categories: Electronics
---
## Introduction
キーボードを作りたくなった。<br>
いまの仕事を抽象化すれば、キーボードを叩く仕事です。ただ、ノートPCのパンタグラフ式キーボードは打鍵音が高くてうるさい。<br>
転職して組み込み半導体に関する仕事をするようになったので、USBペリフェラルの勉強もかねて自作キーボードに挑戦した。<br><br>

---
## 1. 構想と勉強
キー配置を決め、キーボードの仕様を固めていく。また、ソフトウェア開発で重要となるUSB通信の基礎を学ぶ。<br>
...
### 1.2. 部品選定
#### キーボード部品の選定
自作キーボードの一番の楽しみは、[遊舎工房](https://shop.yushakobo.jp/)でお気に入りのキースイッチとキーキャップを探すことだと思います。<br>
僕は"[LeleLab Supsup SuperX White Keycap](https://shop.yushakobo.jp/products/10706?variant=51141650972903)"というスケルトンのキーキャップに魅了されたので、これに合うキースイッチを選んだ。<br>
キースイッチとキーキャップ、スタビライザーを購入して、合計13,694円。<br><br>

#### 回路部品の選定
まず、回路の主要部品となるMCUを選定した。JLCPCBのPCBAサービスを利用してPCBを作製するので、そのサービスで利用できる部品の中から選定する。また、後述のファームウェア開発でQMKを使いたいので、[対応MCU一覧](https://docs.qmk.fm/compatible_microcontrollers)に記載されている**STM32F072**を採用した。<br>
他の部品については、以下のとおりです。<br>

| Category | Parts Number | Detail |
|-|-|-|
| MCU | STM32F072VBT6 | 外付け水晶振動子（48MHz）無しでUSB機能を使える。ピン数が多いパッケージ（LQFP-100）なのでパターン設計が楽（&larr;めっちゃ大事！）。 |
| USB | U262-161N-4BVC11 | USB-Cのレセプタクル。この世の全てのケーブルをtype-Cで統一したい |
| LDO | LM3940IMPX-3.3/NOPB | USB電源からMCU電源を生成する低損失のLDO。出力電流は1Aもいらないと思うけど、ヒートシンク無しだから余裕をもって |
| ESD | ESD122DMXR | USBのデータラインのESD対策 |
| Schottky Diode | SS34 | USB電源（VBUS）への逆流防止用 |
| Diode | 1N1418W | キーマトリクス用の整流ダイオード |

<br>

### 1.3 ファームウェア
自作キーボードはハードウェア（HW）設計がメインで、ファームウェア（FW）は[QMK](https://docs.qmk.fm/)というオープン・ソースのFWを使うのが一般的らしい。キー配置のjsonを作成してビルドすれば、FWを簡単に生成できるので、FW開発の面倒さがない。最高やん！<br>
僕自身、FWへのこだわりはなく、マクロ機能を追加したいくらい。QMKで十分である。<br><br>

#### USB機器の勉強
QMKを使うなら、USBペリフェラルの動作を気にする必要はないですが、せっかくなのでUSBの基礎知識を以下の資料で学んだ。<br><br>
...
## 2. ハードウェア作製
PCB設計・発注、ケース3Dプリントまでの製作過程をまとめる。大まかな流れは以下のとおりです。<br>
1. KiCADで回路図設計
1. KiCADでPCBレイアウト設計
1. [JLCPCB](https://jlcpcb.com/)で基板作製の注文
...
### 2.1 回路設計
まず、回路図を設計する。<br>
KiCADを触るのは8年ぶりなので、使い方の勉強から始める。調べながらやっていくよりも体系的にまとまった資料を読んだ方が早いので、適当な入門書を借りてHands-Onした。<br>
キーボードの回路はシンプルなので、自作キーボード界隈の先人の回路図を読めば、必要な機能を把握できる。<br>
私の作成した回路図も[GitHub](https://github.com/h4rutank/CrazySpring_KB/blob/main/schematic.pdf)にあげているので、参考になれば幸いです。<br>
...
#### 2.2 レイアウト設計
次に、PCBのレイアウトを設計する。キーボードのレイアウト設計で便利なツールを紹介する。<br>
...
ちなみに、回路設計の嫌いな作業ランキング同率1位「パターン設計」は回避できません。数が多いから面倒くさい。<br>
...
JLCPCBはPCBを作るだけでなくて、そのPCBに電子部品をはんだ付けしてくれる**PCBAサービス**をやっている。
自分でチップコンデンサやICを購入して、はんだ付けするコストを考えると、JLCPCBに任せる方が安上がりなので、今回はPCBAサービスを利用する。
PCBAサービスの利用方法は、[JH4VAJさんのブログ](https://www.jh4vaj.com/archives/33375)を参考にしてください。<br>
KiCADでガーバデータとBOMを作成したら、JLCPCBの注文画面でアップロードするだけ。<br>

参考までに、PCBの作成が\$20.6、PCBアセンブリサービスが\$101.34で、送料とクーポンを合わせて\$126.14（=20,725円）だった。（財布が）厳しいって。<br>
...
## 3. ソフトウェア作成
QMKと自作ファームウェア、それぞれの開発をまとめる。<br>
QMKを使うつもりだったが、接続周りのデバッグをしていたら、結局ファームウェアも自作した。詳細は省略する。<br>

### 3.1. QMK
QMKは、自作キーボード向けのオープンソースファームウェアフレームワーク。<br>
MCUのピン配置やキーマトリクスなどのハードウェア情報`keyboards/`ディレクトリに、キー配置やレイヤー・マクロなどのロジックを`keymap.c`に定義して、`qmk compile`コマンドでビルドすれば、FWが完成する。コードを書かなくていいけど、定義ファイルの量が多すぎて、自動化スクリプトが欲しい。<br>
...
### 3.2. 自作ファームウェア
次節の[4. つまづき](#4-つまづき)のとおり、開発の途中で発生した問題の解決のため、FWを自作した。ソースは[GitHub](https://github.com/h4rutank/CrazySpring_KB)で公開している。<br><br>
...
特徴的な機能は
* **JIS配列**<br>ISO60%の物理配置はそのままに、キーコードだけJIS配列へ割り当てている。キーキャップはUS配列なので、キーコードとキーキャップの動作が一致するように頑張って設定した。
* **マクロ**<br>特定のキーを押すと、複数のキー入力を連続で送信できる。ログイン処理や、よく使う定型文の入力を1キーに割り当てている。
* **レイヤーキー**<br>Fnキーを押している間だけ、他のキーの役割を切り替えられる。60%レイアウトで足りない機能をこのレイヤーに割り当てて補っている。
<br><br>
...
---
## Summary
キーボードのキーキャップ以外を自作した。茶軸の「スコスコ」とした打鍵音が良い。<br>IKEA効果も相まって、気に入っている。<br>

...総額40,156円。あれ？HHKBを購入できる金額では？<br>
これが、大量生産の妙か。<br>